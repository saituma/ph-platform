...
export default function ParentPlatformScreen() {
  ...
  const lockedCount = filteredItems.filter(
    (item) =>
      !canAccessTier(programTier, item.programTier ?? null) && !item.isPreview,
  ).length;
  ...
  const openCourse = (item: ParentCourseItem) => {
    ...
    if (isLocked) {
      const unlockingPlans = getUnlockingPlanNames(
        ...
        >
          ...
          <View className="mt-5 flex-row gap-3">
            <MetricCard
              ...
              caption="Available to browse now"
              isDark={isDark}
            />
            <MetricCard
              title="Previews"
        ...
        {!hasParentProgramAccess ? (
            ...
            <View className="mt-4 gap-3">
              {[
                ...
                <View key={benefit} className="flex-row items-start gap-3">
                  ...
                  <Text className="flex-1 text-sm font-outfit text-app leading-6">
                    {benefit}
                  </Text>
                </View>
              ))}
          ...
          <View className="gap-3">
            {[1, 2, 3].map((item) => (
              <View
                key={item}
                className="rounded-3xl border border-app/10 bg-input px-4 py-4"
              >
          ...
          <View className="gap-8">
            ...
            {filteredItems.length === 0 ? (
              <View className="rounded-[28px] border border-dashed border-app/20 p-5">
                <Text className="mb-2 font-clash text-xl text-app">
                  No matching courses
                </Text>
                <Text className="text-base font-outfit leading-6 text-secondary">
                  Try a broader term like recovery, nutrition, sleep, growth, or
                  confidence.
                </Text>
              </View>
            ) : null}
        ...
        >
          <View className="mb-3 flex-row items-center justify-between gap-3">
            <View className="flex-row items-center gap-3">
              ...
              <View>
                <Text className="font-clash text-lg font-bold text-accent">
                  Full Access
                </Text>
                <Text className="text-base font-outfit text-secondary">
                  More support across every topic
                </Text>
              </View>
            </View>
            <View
...
function SectionLabel({ label }: { label: string }) {
  return (
    <Text className="mb-4 ml-2 text-[11px] font-outfit font-bold uppercase tracking-[1.6px] text-secondary">
      {label}
    </Text>
  );
}
...
function MetricCard({
  ...
  return (
    ...
    >
      <Text className="mb-2 text-[10px] font-outfit font-bold uppercase tracking-[1.3px] text-secondary">
        {title}
      </Text>
      <Text className="mb-1 font-clash text-2xl text-app">{value}</Text>
      <Text className="text-base font-outfit text-secondary leading-5">
        {caption}
      </Text>
    </View>
  );
    ...
    >
      <View className="flex-row items-start justify-between gap-3">
        <View className="flex-1">
          ...
          <Text
            className="text-sm font-outfit text-secondary leading-6"
            numberOfLines={3}
          >
            {item.summary}
          </Text>
      ...
      <View className="mt-4 flex-row items-center justify-between">
        ...
        <View className="flex-row items-center gap-2">
          {isLocked ? (
            ...
            >
              <Text className="text-[10px] font-outfit font-bold uppercase tracking-[1.1px] text-secondary">
                Locked
              </Text>
            </View>
          ) : null}
...
function CourseCard({
  ...
  return (
    ...
    >
      <View className="flex-row items-start justify-between gap-3">
        <View className="flex-1">
          <Text className="font-outfit text-base font-bold text-app">
            {item.title}
          </Text>
      ...
      <Text
        className="mt-3 text-sm font-outfit text-secondary leading-6"
        numberOfLines={3}
      >
        {item.summary}
      </Text>
