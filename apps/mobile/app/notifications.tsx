...
export default function NotificationsScreen() {
  ...
  type FlatItem =
    ...
    | { type: "group"; key: string; group: NotificationGroup; index: number };

  const flatListData = useMemo<FlatItem[]>(() => {
    const result: FlatItem[] = [];
    for (const section of sections) {
      result.push({ type: "section", key: `s-${section.key}`, title: section.title });
      section.groups.forEach((group, idx) => {
        result.push({ type: "group", key: `g-${section.key}-${group.id}-${idx}`, group, index: idx });
      });
    }
    return result;
  }, [sections]);

  const renderListItem = useCallback(({ item }: { item: FlatItem }) => {
    ...
    return (
      <Animated.View entering={FadeInDown.delay(Math.min(index * 35, 350)).duration(200)}>
        <Pressable
          onPress={() => {
            group.items.forEach((n) => { if (!n.read) markRead(n.id); });
            if (group.link) router.navigate(group.link as any);
          }}
          style={({ pressed }) => ({
        ...
        >
          <View style={{ backgroundColor: p.cardWhite, borderRadius: 22, padding: 16 }}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <View style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: accentSoft, alignItems: "center", justifyContent: "center", marginRight: 14 }}>
                <IconComp size={20} color={accent} />
              </View>
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  <Text style={{ fontSize: 15, fontFamily: "Outfit-Bold", color: p.textPrimary }}>{group.title}</Text>
                  {group.count > 1 ? (
                    <View style={{ paddingHorizontal: 8, paddingVertical: 2, borderRadius: 100, backgroundColor: p.accentSoft }}>
                      <Text style={{ fontSize: 11, fontFamily: "Outfit-Bold", color: p.accent }}>{group.count}</Text>
                    </View>
                  ) : null}
                </View>
                <Text numberOfLines={2} style={{ fontSize: 14, fontFamily: "Outfit-Regular", color: p.textSecondary, marginTop: 4 }}>{group.message}</Text>
                <View style={{ flexDirection: "row", alignItems: "center", marginTop: 12 }}>
                  {group.unreadCount > 0 ? <View style={{ width: 8, height: 8, borderRadius: 999, backgroundColor: accent, marginRight: 6 }} /> : null}
                  <Text style={{ fontSize: 12, fontFamily: "Outfit-Regular", color: p.textMuted }}>{formatRelativeTime(group.latestAt)}</Text>
                  {group.unreadCount > 0 ? (
                    <Text style={{ fontSize: 12, fontFamily: "Outfit-Regular", color: p.textMuted, marginLeft: 6 }}>· {group.unreadCount} unread</Text>
                  ) : null}
                </View>
              </View>
            </View>
