...
export default function TermsScreen() {
  ...
  const { data } = useQuery({
    queryKey: queryKeys.legal.terms(authed),
  ...
  const termsContent = data?.body ?? cachedData?.body ?? null;
  const termsVersion = data?.version ?? cachedData?.version ?? null;
  const termsUpdatedAt = data?.updatedAt ?? cachedData?.updatedAt ?? null;
    ...
    <View style={{ flex: 1, paddingTop: insets.top, backgroundColor: p.pageBg }}>
      ...
      >
        ...
        <View style={{ gap: 24 }}>
          ...
          >
            <MarkdownText
              text={(termsContent && termsContent.trim().length ? termsContent : FALLBACK_CONTENT).trim()}
