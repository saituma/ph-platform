...
export default function AdminVideosScreen() {
  ...
  // Prefetch the first 10 video URLs into the shared LRU disk cache via the
  // bounded-concurrency queue so they're instant when the admin opens them.
  // The old ad-hoc createDownloadResumable loop bypassed dedup and LRU.
  const prefetchTargets = useMemo(
    () => items.slice(0, 10).map((it) => ({ url: it.videoUrl })),
    [items],
  );
