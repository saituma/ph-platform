...
import { View, FlatList, KeyboardAvoidingView, Platform, Pressable, Keyboard } from "react-native";
...
export const ThreadChatBody = React.memo(function ThreadChatBody({
  ...
  const { listRef, handleScroll, jumpTo, newIncomingCount, highlightedId } = useChatScroll(messages, thread.id);
  ...
  const renderItem = useCallback(({ item, index }: { item: ChatMessage; index: number }) => (
    <Animated.View entering={FadeInDown.delay(Math.min(index * 50, 400))} layout={Layout.springify()}>
      <MessageBubble
        message={item}
        token={token}
  ...
  return (
    ...
    >
      <FlatList
        ...
        contentContainerStyle={{ paddingHorizontal: 12, paddingTop: 100, paddingBottom: insets.top + 90, rowGap: 12 }}
        ...
        ListFooterComponent={
          <ChatIntroHeader
            ...
            statusText={typing?.isTyping ? "Typing..." : (thread.lastSeen ?? "Active")}
          />
        }
      />
      ...
      {newIncomingCount > 0 && (
        ...
        >
          <View className="bg-accent px-4 py-2 rounded-full flex-row items-center gap-2 shadow-lg">
            ...
            <Text className="text-white font-outfit-bold text-xs">{newIncomingCount} New</Text>
          </View>
        </Pressable>
      )}
      ...
      <View style={{ position: "absolute", bottom: 0, left: 0, right: 0 }}>
          ...
          <View className="px-4 pb-2">
            <View className="bg-card rounded-2xl p-3 flex-row items-center border border-border">
              <View className="flex-1">
                <Text className="text-[10px] font-bold text-accent uppercase">Replying to {replyTarget.authorName}</Text>
                <Text className="text-xs text-secondary" numberOfLines={1}>{replyTarget.preview}</Text>
              </View>
              <Pressable onPress={onClearReplyTarget}><Feather name="x" size={18} color={colors.textSecondary} /></Pressable>
            </View>
          </View>
        )}

        {typing?.isTyping && <View className="px-5 pb-2"><TypingIndicator /></View>}
        ...
        <ChatComposer
          ...
          placeholder={thread.id.startsWith("group:") ? "Message the group" : "Type a message"}
