...
export const EmbeddedIntroPlayer = React.memo(function EmbeddedIntroPlayer({ kind, url, posterUrl, width, height }: Props) {
  ...
  useEffect(() => () => { if (retryTimer.current) clearTimeout(retryTimer.current); if (kind === "youtube") { try { youtubeRef.current?.pauseVideo?.(); } catch {} void youtubeRef.current?.getCurrentTime?.().then((seconds: number) => writeIntroProgress(url, seconds)); } }, [kind, url]);
  ...
  const handleReady = useCallback(() => { retryCount.current = 0; setReady(true); setError(false); }, []);
