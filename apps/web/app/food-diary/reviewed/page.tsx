...
export default function FoodDiaryReviewedPage() {
  ...
  return (
    <AdminShell title="Food Diary – Reviewed" subtitle="Past entries with coach feedback.">
      <Card>
        <CardContent className="pt-6">
          <SectionHeader
            title="Reviewed"
            description={`${entries.length} entr${entries.length === 1 ? "y" : "ies"} with feedback.`}
          />
          <div className="mt-4">
            <Link
              href="/food-diary"
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="Reviewed entries will appear here."
            />
          ) : (
            <div className="mt-6 space-y-4">
              {entries.map((entry: any) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-foreground">View details and feedback →</p>
                </Link>
              ))}
            </div>
          )}
