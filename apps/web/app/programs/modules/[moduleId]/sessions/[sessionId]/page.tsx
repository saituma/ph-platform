...
type LibraryModuleRecord = { id: number; title?: string | null };
type ProgramSession = {
  id: number;
  title?: string | null;
...
function LibraryPicker({ exercises, search, onSearchChange, onPick, isAdding, emptyReason, onCreateInstead }: LibraryPickerProps) {
  const listId = useId();
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {ex.cues && (
                      <p className="mt-0.5 line-clamp-1 text-xs text-muted-foreground">{ex.cues}</p>
                    )}
                  </div>
...
function SortableExerciseRow({ se, index, onNotes, onRemove, isRemoving }: SortableRowProps) {
  ...
  const hasNotes = se.coachingNotes || se.progressionNotes || se.regressionNotes;

  return (
    <div ref={setNodeRef} style={style} className="flex items-start gap-3 rounded-2xl border border-border bg-card p-4">
      <button
        type="button"
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
          {se.exercise?.category && (
            <Badge variant="secondary" className="text-[10px]">{se.exercise.category}</Badge>
          )}
          {(se.setsOverride != null || se.exercise?.sets != null) && (
            <span className={se.setsOverride != null ? "font-semibold text-primary" : "text-muted-foreground"}>
              ...
              {se.setsOverride != null && se.exercise?.sets != null && se.setsOverride !== se.exercise.sets && (
                <span className="ml-0.5 text-muted-foreground line-through">{se.exercise.sets}</span>
              )}
            </span>
          )}
          {(se.repsOverride != null || se.exercise?.reps != null) && (
            <span className={se.repsOverride != null ? "font-semibold text-primary" : "text-muted-foreground"}>
              ...
              {se.repsOverride != null && se.exercise?.reps != null && se.repsOverride !== se.exercise.reps && (
                <span className="ml-0.5 text-muted-foreground line-through">{se.exercise.reps}</span>
              )}
            </span>
          )}
      ...
      <div className="flex shrink-0 items-center gap-1">
        <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => onNotes(se)} title="Session notes & overrides">
          <Pencil className="h-3.5 w-3.5" />
        </Button>
        <Button variant="ghost" size="sm" className="h-7 w-7 shrink-0 p-0 text-destructive" onClick={() => onRemove(se.id)} disabled={isRemoving}>
...
function SessionNotesDialog({ open, se, onClose, onSave, saving }: SessionNotesDialogProps) {
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {ex?.name ?? "Exercise"}
            <span className="text-sm font-normal text-muted-foreground">— session overrides</span>
          </DialogTitle>
          <DialogDescription>
            These notes are specific to this session and don&apos;t affect the exercise library.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-2 space-y-5">
          <div className="rounded-xl border border-border bg-secondary/30 p-3">
            ...
            {ex?.videoUrl && (
              <div className="mt-3 aspect-video overflow-hidden rounded-lg border border-border">
                <video className="h-full w-full object-cover" src={ex.videoUrl} controls muted preload="metadata" />
              </div>
            )}
            <Link
          ...
          <div className="space-y-2">
            <p className="text-xs font-semibold text-foreground">
              Load overrides{" "}
              <span className="font-normal text-muted-foreground">— leave blank to use the library defaults</span>
            </p>
          ...
          <div className="space-y-4">
            {[
              { label: "Coaching notes", field: "coachingNotes" as const, placeholder: "e.g. Focus on knee tracking today…" },
              { label: "Progression notes", field: "progressionNotes" as const, placeholder: "e.g. Add weight next session…" },
              { label: "Regression notes", field: "regressionNotes" as const, placeholder: "e.g. Use resistance band…" },
            ].map(({ label, field, placeholder }) => (
              <div key={field} className="space-y-1.5">
                <Label className="text-xs">
                  {label}{" "}
                  <span className="text-muted-foreground">(for this session only)</span>
                </Label>
                <Textarea
                  rows={2}
                  placeholder={placeholder}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={() => se && onSave(se.id, form)} disabled={saving}>
              {saving ? "Saving..." : "Save"}
            </Button>
          </div>
        </div>
...
export default function LibrarySessionDetailPage() {
  ...
  const currentModule = useMemo(
    () => (libraryData?.modules ?? []).find((m: LibraryModuleRecord) => m.id === moduleId),
    [libraryData, moduleId],
  );
  ...
  const openLibrary = () => { setLibrarySearch(""); setAddDialog("library"); };
  const openCreate = () => { setExerciseForm(emptyForm); setAddDialog("create"); };
  const closeAddDialog = () => { setAddDialog(null); setExerciseForm(emptyForm); };
    ...
    <AdminShell
      ...
      subtitle={
        <span className="flex items-center gap-2 text-xs">
          ...
          <Link href="/programs/modules" className="text-muted-foreground hover:text-foreground">Module Library</Link>
          <ChevronRight className="h-3 w-3 text-muted-foreground" />
          <Link href={`/programs/modules/${moduleId}`} className="text-muted-foreground hover:text-foreground">
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border bg-card p-8 text-center text-sm text-muted-foreground">
          ...
          <p className="mt-1">Click &quot;Add Exercise&quot; to pick from your library or create a new one.</p>
        </div>
      ) : (
      ...
      <Dialog open={addDialog !== null} onOpenChange={(open) => { if (!open) closeAddDialog(); }}>
