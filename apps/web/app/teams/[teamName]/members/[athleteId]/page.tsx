...
export default function TeamMemberDetailPage() {
  ...
  const loadOnboardingFallback = async () => {
    const usersResponse = await fetch("/api/backend/admin/users", { credentials: "include" });
    const usersPayload = await usersResponse.json().catch(() => ({}));
    if (!usersResponse.ok) return null;

    const matchedUser = (Array.isArray(usersPayload?.users) ? usersPayload.users : []).find(
      (user: AdminUser) => user.role === "guardian" && Number(user.athleteId) === athleteId
    );
    if (!matchedUser?.id) return null;

    const onboardingResponse = await fetch(`/api/backend/admin/users/${matchedUser.id}/onboarding`, { credentials: "include" });
  ...
  const loadMember = async () => {
    ...
    try {
      ...
      const needsFallback = !next.injuries && !next.growthNotes && !next.performanceGoals && !next.equipmentAccess;
      const fallbackAthlete = needsFallback ? await loadOnboardingFallback() : null;
      ...
      setForm({
        ...
        growthNotes: parseLines(next.growthNotes ?? fallbackAthlete?.growthNotes ?? ""),
        performanceGoals: parseLines(next.performanceGoals ?? fallbackAthlete?.performanceGoals ?? ""),
        equipmentAccess: parseLines(next.equipmentAccess ?? fallbackAthlete?.equipmentAccess ?? ""),
    ...
    try {
      ...
      const response = await fetch(`/api/backend/admin/teams/${encodeURIComponent(teamName)}/members/${athleteId}`, {
        ...
        body: JSON.stringify({
          ...
          performanceGoals: joinLines(form.performanceGoals).join("\n") || null,
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            {isLoading ? (
              ...
              <>
                ...
                {renderMultiField("Injuries / history", form.injuries, (next) => setForm((current) => ({ ...current, injuries: next })), "sm:col-span-2")}
                {renderMultiField("Growth notes", form.growthNotes, (next) => setForm((current) => ({ ...current, growthNotes: next })))}
                {renderMultiField("Performance goals", form.performanceGoals, (next) => setForm((current) => ({ ...current, performanceGoals: next })))}
                {renderMultiField(
                  "Equipment access",
                  form.equipmentAccess,
                  (next) => setForm((current) => ({ ...current, equipmentAccess: next })),
                  "sm:col-span-2"
                )}
