...
async function request<T>(url: string, init: RequestInit = {}, options: RequestOptions = {}): Promise<T> {
	...
	for (let attempt = 0; attempt <= retries; attempt++) {
		...
		try {
			const response = await csrfFetch(url, { ...init, signal: controller.signal, credentials: "include" });
...
export const api = {
	...
	post: <T>(url: string, body?: unknown, options?: RequestOptions) =>
		request<T>(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: body ? JSON.stringify(body) : undefined }, options),
	put: <T>(url: string, body?: unknown, options?: RequestOptions) =>
		request<T>(url, { method: "PUT", headers: { "Content-Type": "application/json" }, body: body ? JSON.stringify(body) : undefined }, options),
	patch: <T>(url: string, body?: unknown, options?: RequestOptions) =>
		request<T>(url, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: body ? JSON.stringify(body) : undefined }, options),
