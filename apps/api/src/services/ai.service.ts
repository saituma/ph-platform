...
export async function generateAiCoachResponse(userMessage: string, history: { role: "user" | "assistant", content: string }[] = []) {
  ...
  try {
    ...
    return response.choices[0]?.message?.content?.trim() ?? "I'm having trouble thinking right now. Let's try again in a moment.";
  } catch (error) {
