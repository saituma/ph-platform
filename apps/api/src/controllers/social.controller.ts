...
export function handleSocialError(res: Response, err: unknown) {
...
export async function commentsList(req: Request, res: Response) {
  ...
  try {
    ...
    const items = await listRunComments(req.user.id, Math.floor(runLogId), undefined);
    return res.status(200).json({ items });
  } catch (err) {
    return handleSocialError(res, err);
  }
