		...
		const animate = (currentTime: number) => {
			...
			switch (animationState) {
				...
				case "vaporizing": {
					...
					if (
						vaporizeProgressRef.current >= 100 &&
						allVaporized
					) {
						setCurrentTextIndex(
				...
				case "fadingIn": {
					...
					for (const particle of particlesRef.current) {
						...
						const clr = particle.color.replace(
							/[\d.]+\)$/,
							`${opacity})`,
						);
						ctx.fillStyle = clr;
...
const renderCanvas = ({
	...
	const currentText =
		framerProps.texts?.[currentTextIndex] || "Next.js";
...
const createParticles = (
	...
	const baseSampleRate = Math.max(
		1,
		Math.round(currentDPR / baseDPR),
	);
	...
	for (let y = 0; y < canvas.height; y += sampleRate) {
		for (let x = 0; x < canvas.width; x += sampleRate) {
			...
			if (alpha > 0) {
				const originalAlpha =
					(alpha / 255) * (sampleRate / currentDPR);
				const particle = {
					x,
					y,
					originalX: x,
	...
	for (const particle of particles) {
		...
		if (shouldVaporize) {
			if (particle.speed === 0) {
				...
				particle.shouldFadeQuickly = Math.random() > dens;
			}
			...
			if (particle.shouldFadeQuickly) {
				...
				const dampingFactor = Math.max(
					...
					1 -
						distanceFromOrigin /
							(100 * MULTIPLIED_VAPORIZE_SPREAD),
				);
				...
				particle.velocityX =
					(particle.velocityX + spreadX + dx * 0.002) *
					dampingFactor;
				particle.velocityY =
					(particle.velocityY + spreadY + dy * 0.002) *
					dampingFactor;
