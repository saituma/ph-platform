...
function PortalInner() {
	...
	const onboardingIncomplete = isTeam ? !user.team?.id : !hasActivePlan && !user.onboardingCompleted;
...
function OnboardingGate({ user, isTeam, onboardingIncomplete, needsPlan }: {
	...
	useEffect(() => {
		...
		async function loadBillingStatus() {
			try {
				...
				if (cancelled) return;
				setLatestRequestStatus(String((data as any)?.latestRequest?.status ?? "").trim().toLowerCase() || null);
			} catch {
				// best effort only
			}
		}
	...
	const nextStep = useMemo(() => {
		...
		return 4;
	}, [onboardingIncomplete, isTeam, user.birthDate, user.trainingPerWeek, user.performanceGoals, user.phoneNumber, user.equipmentAccess]);
	...
	const primaryAction = () => {
		if (inReview) {
			toast.message("Your payment is under review.", { description: "Your coach/admin will approve access shortly." });
			return;
		}
		...
		<div className="min-h-screen flex items-center justify-center px-4 py-12">
				...
				{!onboardingIncomplete && needsPlan && (
					<div className="mt-6 border border-foreground/[0.06] bg-foreground/[0.02] px-4 py-3 text-sm text-muted-foreground">
						{inReview ? "Payment received. Approval pending." : "No active subscription found. Choose a plan to continue."}
					</div>
				)}
